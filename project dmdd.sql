...
3) Inserting Data into Tables
	...
	- Hiring Period
	- Job Card
4) Indexing (6*Indexes)
5) Stored Procedure (4*SP)
6) Triggers (1*Trigger)
7) UDF and Computed Column Definition Function (1 UDF and CCDF) 
...
--Government(GovtID,ServingPeriod)
CREATE TABLE GOVERNMENT(
    GovtId INT NOT NULL,
    ServingPeriod INT
    CONSTRAINT GOVERNMENT_PK PRIMARY KEY(GovtId)
)
...
CREATE TABLE [YEAR](
    ...
    CONSTRAINT YEAR_FK1 FOREIGN KEY(GovtId)
    REFERENCES GOVERNMENT(GovtId)
)

--Region(RegionId,State,District,GovtId)
CREATE TABLE REGION(
    ...
    CONSTRAINT REGION_FK1 FOREIGN KEY(GovtId)
    REFERENCES GOVERNMENT(GovtId)
)

--Expenditure(ExpenditureId,Labour,GovtId,Year)
CREATE TABLE EXPENDITURE(
    ExpenditureId INT NOT NULL,
    Labour INT not null,
    GovtId INT NOT NULL,
    [Year] INT NOT NULL,
    CONSTRAINT EXPENDITURE_PK PRIMARY KEY(EXPENDITUREID),
    CONSTRAINT EXPENDITURE_FK1 FOREIGN KEY(GovtId)
    REFERENCES GOVERNMENT(GovtId),
    CONSTRAINT EXPENDITURE_FK2 FOREIGN KEY([Year])
    REFERENCES [YEAR]([Year])
)

--Caste(CasteID, Year)
CREATE TABLE Caste(
	CasteID int primary key not null,
	[Type] varchar(25) CHECK (Type='Bhramin'or Type='Dalit'or Type='Kshatriya'or Type='Vaishyas'or Type='Shudras') 
);

--Person(Aadhaar, Name, CasteID, Duration, Gender, UPIID, DOB, RegionID)
CREATE TABLE Person(
	Aadhaar int primary key not null,
	[Name] VarChar(30),
	...
	RegionID int not null,
	Constraint Person_FK1 foreign key (RegionID) references Region (RegionID),
    CONSTRAINT Person_FK2 FOREIGN KEY (CasteId) REFERENCES Caste(CasteID)   
);

--BankAccount(BankID, AccountNumber, TypeOfAccount, Aadhaar,)
CREATE TABLE BankAccount(
	BankID BIGINT primary key not null,
	AccountNumber BIGINT,
	TypeOfAccount VarChar(25),
	Aadhaar int not null,
	Constraint BANKACCOUNT_FK foreign key (Aadhaar) references Person (Aadhaar)
);

--Salary(SalaryID, Amount, AmountDue, Aadhar)
CREATE TABLE Salary(
    SalaryID BIGINT not null,
    Amount BIGINT,
    AmountDue BIGINT,
    Aadhar INT not null,
    CONSTRAINT Salary_PK1 PRIMARY KEY (SalaryID),
    CONSTRAINT Salary_FK FOREIGN KEY (Aadhar) REFERENCES Person
);

--Jobs(JobsID, JobType,JobName,Requirements,SalaryID, CasteID)
CREATE TABLE Jobs(
    JobsID int not null,
    JobType VARCHAR(100),
    JobName VARCHAR(100),
    ...
    CONSTRAINT Jobs_FK1 FOREIGN KEY (SalaryID) REFERENCES Salary,
    CONSTRAINT Jobs_FK2 FOREIGN KEY (CasteID) REFERENCES Caste
);

--JobCard(JobCardID, Applied, Issued, Aadhar, JobsID)
CREATE TABLE JobCard(
    ...
    Aadhar INT not null,
    JobsID INT not null,
    CONSTRAINT JobCard_PK PRIMARY KEY (JobCardID),
    CONSTRAINT JobCard_FK1 FOREIGN KEY (Aadhar) REFERENCES Person,
    CONSTRAINT JobCard_FK2 FOREIGN KEY (JobsID) REFERENCES Jobs,
);

--HiringPeriod(CasteID, JobsID, TimeHiredFor)
CREATE TABLE HiringPeriod(
    CasteID INT not null,   
    JobsID INT not null,
    TimeHiredFor INT not null,
    CONSTRAINT HiringPeriod_PK PRIMARY KEY (CasteID,JobsID),
    CONSTRAINT HiringPeriod_FK1 FOREIGN KEY (CasteID) REFERENCES Caste,
    CONSTRAINT HiringPeriod_FK2 FOREIGN KEY (JobsID) REFERENCES Jobs,
);

--Expenditure(UPIID,UPI_Expenditure_Total)
CREATE TABLE UPI_EXPENDITURE(
    UPIID VARCHAR(30) NOT NULL,
    UPI_Expenditure_Total INT,
    CONSTRAINT UPI_EXPENDITURE_PK PRIMARY KEY (UPIID)
);

-- Altering Person Table to add UPI ID
ALTER TABLE Person ADD CONSTRAINT Person_FK3 FOREIGN KEY (UPIID) REFERENCES UPI_EXPENDITURE (UPIID);
...
SELECT * FROM Jobs
SELECT * FROM JobCard
SELECT * FROM HiringPeriod
SELECT * FROM UPI_EXPENDITURE
...
CREATE INDEX region_idx ON Region ([State]);
CREATE INDEX person_idx1 ON Person ([Name]);
CREATE INDEX person_idx2 ON Person (Duration);
CREATE INDEX jobcard_idx1 ON Jobcard (applied);
CREATE INDEX jobcard_idx2 ON Jobcard (issued);
CREATE INDEX jobs_idx ON Jobs (jobname);
Go

/*###############################################  PART 5 - Stored Procedure ##############################################################*/
--1) JOBCARD TIME TAKEN STORED PROCEDURE
CREATE PROCEDURE GetTimeTakenForJobCard AS
BEGIN
    SELECT JC.JobCardID,
    DATEDIFF(day, Applied, Issued) AS TimeTakenInDays
    FROM JobCard JC
END
Go
EXEC GetTimeTakenForJobCard
GO


--2) GENDER EMPLOYEE SKILLED UNSKILLED
CREATE PROCEDURE GetEmmployee @gender VARCHAR(30), @jobtype VARCHAR(30) AS
BEGIN
SELECT  COUNT(Person.Gender) AS EmployeeCount FROM Person
INNER JOIN JobCard ON Person.Aadhaar=JobCard.Aadhar
INNER JOIN Jobs ON JobCard.JobsID=Jobs.JobsID
WHERE Person.Gender=@gender AND Jobs.JobType=@jobtype 
END 
GO
EXEC GetEmmployee 'F','UnSkilled'
Go


--3) UPI_EXPENDITURE STORED PROCEDURE
CREATE PROCEDURE FrequentUpiExpenditure AS
SELECT P.Name,P.UPIID,upi.UPI_Expenditure_Total 
FROM Person P
INNER JOIN UPI_EXPENDITURE upi ON P.UPIID=upi.UPIID
WHERE upi.UPI_Expenditure_Total>500
Go
EXEC FrequentUpiExpenditure
GO


--4) TIME PERIOD HIRING PERIOD
CREATE PROCEDURE GetLongestTimePeriod @LongestTime DECIMAL OUTPUT
AS
DECLARE @MaxTime DECIMAL
BEGIN
    SELECT @MaxTime= MAX(Person.Duration) FROM Person
    SET @LongestTime = @MaxTime
END
DECLARE @LongestTime DECIMAL
EXEC GetLongestTimePeriod @LongestTime =@LongestTime OUTPUT
SELECT @LongestTime VALUE_AS_OUTPUT_PARAMETER
PRINT 'THE LONGEST PERIOD OF TIME A PERSON WORKED IS ' + CAST(@LongestTime as varchar) + ' Years'
...
select * from Person;
Go
CREATE TRIGGER TriggerPerson ON PERSON
    AFTER INSERT 
AS
BEGIN
    SET NOCOUNT ON
    DECLARE @Aadhaar INT
    SELECT @Aadhaar = inserted.Aadhaar
    from
    INSERTED

    INSERT INTO PersonLogs
    VALUES(@Aadhaar,'New Person Added',SYSDATETIME())
...
/*######################################## PART 7 - UDF and Computed Column Definition Function ###########################################*/
CREATE FUNCTION AvgJobExperience
(
    @jobname VARCHAR(30)
)
RETURNS VARCHAR(30)
...
BEGIN
    DECLARE @experience DECIMAL(2,1)
        SELECT @experience = AVG(Person.Duration) 
    FROM Jobs
    INNER JOIN JobCard ON Jobs.JobsID=JobCard.JobsID
    INNER JOIN Person ON Person.Aadhaar=JobCard.Aadhar
    WHERE Jobs.JobName=@jobname
    --SET @c
    RETURN @experience 

END
GO
SELECT dbo.AvgJobExperience('Data Entry') AS AvgJobExperienceInYears
GO

/*#################################### PART 8 - Encryption and Decryption #################################################################*/

/******************Encryption************/
--Create a master key for the database. 
create MASTER KEY
ENCRYPTION BY PASSWORD = 'Dmddgroup5';
...
CREATE CERTIFICATE Group5project  
   WITH SUBJECT = 'Project Sample Password';  
GO  
...
CREATE SYMMETRIC KEY Group5project_SM 
    WITH ALGORITHM = AES_256  
    ENCRYPTION BY CERTIFICATE Group5project;  
GO  
...
OPEN SYMMETRIC KEY Group5project_SM  
   DECRYPTION BY CERTIFICATE Group5project;  



-- Encrypt the value in column Password  with symmetric  key, and default everyone with
-- a password of Pass1234  
UPDATE dbo.Person set  [UpiEncrypt]  = EncryptByKey(Key_GUID('Group5project_SM'),Person.UPIID )   
GO  

CLOSE SYMMETRIC KEY Group5project_SM;
GO
/******************Decryption************/
OPEN SYMMETRIC KEY Group5project_SM
        DECRYPTION BY CERTIFICATE Group5project;
...
SELECT Aadhaar,[Name],[UpiEncrypt],
            CONVERT(varchar, DecryptByKey(UpiEncrypt)) AS 'UpiDecrypted'
            FROM Person;
...
--1) CASTE VIEW
CREATE VIEW [Caste with max Jobs] AS
SELECT Caste.[Type] AS Caste,COUNT(JobsID) AS JobsOffered
FROM HiringPeriod
INNER join Caste on Caste.CasteID= HiringPeriod.CasteID
GROUP BY Caste.[Type]
GO
SELECT * FROM [Caste with max Jobs]
Go


--2) GOVERNMENT EXPENDITURE VIEW
CREATE VIEW [Government with max Expenditure] AS
SELECT g.GovtId, SUM(e.Labour) as ToatlExpenditure
FROM GOVERNMENT g
INNER join EXPENDITURE e on g.GovtId=e.GovtId
GROUP BY g.GovtId
GO
SELECT * FROM [Government with max Expenditure]
Go



--3) ZERO AMOUNT DUE
CREATE VIEW [SalaryCleared] AS
SELECT PERSON.Name AS SalaryCLeared FROM Salary
INNER JOIN Person ON Person.Aadhaar = Salary.Aadhar
WHERE AmountDue=0
GO
SELECT * FROM [SalaryCleared]
